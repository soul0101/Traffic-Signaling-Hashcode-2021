...
def solveAll(filename):
    ...
    traffic = []
    intersectionFromName = defaultdict(int)

    frequency_end = [0]*(len(streets))
    frequency_in = [0]*(len(streets))
    frequency_street = defaultdict(int)
    inc = defaultdict(list)

    for street in streets:
        frequency_end[street.E] += 1
        frequency_in[street.B] += 1
        intersectionFromName[street.name[0]] = street.E
    #print(intersectionFromName)
    ...
    check=len(new_frequency)//2
    c=0
    for key, value in new_frequency:      
      if c>check:
        break
      c+=1
      temp = Light(intersectionFromName[key],key,3)    
      if(temp not in traffic):
        traffic.append(
          temp
        )
    ...
    for i in traffic:
      if([i.street,i.time] not in sched[i.intersection]):
